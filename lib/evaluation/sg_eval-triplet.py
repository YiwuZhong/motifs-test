...
def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, multiple_preds=False,
    ...
    rel_pred = pred_entry['sorted_rel_pred']
    ...
    if multiple_preds:
        ...
        pred_rels = np.column_stack((pred_rel_inds, rel_pred))
...
def evaluate_recall(gt_rels, gt_boxes, gt_classes,
    ...
    # pred_rels: [#pred_rel, 3] array of pred rels. Assumed these are in sorted order and refer to IDs in pred classes / pred boxes
    #(id0, id1, rel)
    if pred_rels.size == 0:
    ...
    """
    # return prod(uct) of elements of each row (axis = 1, product each column)
    # (num_rel, 3) --> (num_rel, 1)
    scores_overall = relation_scores.prod(1)
    # print("scores_overall is  ", scores_overall)
    ...
    # pred_to_gt: list len is num_rel
    # pred_5ples: (num_rel, 5), (id0, id1, cls0, cls1, rel)
    # relation_scores: (num_rel, 3), (box1, box2, predicate score)
