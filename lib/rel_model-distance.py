...
class RelModel(nn.Module):
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        im_bboxes = torch.cat((im_inds[:, None].float(), bboxes), 1)
        new_obj_fmap = self.new_obj_feature_map(result.fmap.detach(), im_bboxes.detach())  # [384,4096]
        obj1 = self.obj1_fc(new_obj_fmap[rel_inds[:, 1]])
        obj1 = obj1.view(obj1.size(0), self.num_classes, self.embdim)  # (275, 151, 10)
        obj2 = self.obj2_fc(new_obj_fmap[rel_inds[:, 2]])
        obj2 = obj2.view(obj2.size(0), self.num_classes, self.embdim)  # (275, 151, 10)
        ######################### box + deltas resample fmap ##############################
        """
        ...
        if self.training:
            twod_inds1 = arange(result.rm_obj_preds.data[rel_inds[:, 1]]) * self.num_classes + result.rm_obj_preds.data[rel_inds[:, 1]]
            twod_inds2 = arange(result.rm_obj_preds.data[rel_inds[:, 2]]) * self.num_classes + result.rm_obj_preds.data[rel_inds[:, 2]]
            rel_type = result.rel_labels[:, 3].data.clone()  # [275]
            twod_inds_r = arange(rel_type) * self.num_rels + rel_type
            # distance: [275, 10]
            distance = obj1.view(-1,self.embdim)[twod_inds1] + rel_emb.view(-1,self.embdim)[twod_inds_r] - obj2.view(-1,self.embdim)[twod_inds2]
            #a = LA.norm(distance.data.cpu().numpy(), axis=1)   # 'elementwise_mean' -> scalar
            #b = torch.mean(torch.from_numpy(a)).cuda()  # important
            #c = Variable(b) 
            result.distance = torch.sqrt(distance.pow(2).sum(1))  # faster than torch.norm()
