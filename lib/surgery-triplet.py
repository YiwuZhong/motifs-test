...
def filter_dets(boxes, obj_scores, obj_classes, rel_inds, pred_scores, obj1, obj2, rel_emb):
    ...
    obj_pairs_scores = obj_scores0 * obj_scores1
    _, obj_pairs_idx = torch.sort(obj_pairs_scores.view(-1), dim=0, descending=True)
    rel_inds = rel_inds[obj_pairs_idx]
    ##################### overall score part #######################
    ...
    # rel_distance_vs: sorted distance, double tensor; rel_distance_idx: long tensor
    #_, rel_distance_idx = torch.sort(distance.contiguous().view(-1), dim=0, descending=False)
    ...
    rels = rel_inds.cpu().numpy()  # rel_inds is "cuda" long tensor
    sorted_rel_pred = rel_pred.cpu().numpy().astype(int)  # if it's float, when column stack it with rels, the entity will be float
